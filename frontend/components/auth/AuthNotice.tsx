...
type AuthNoticeVariant = "error" | "success" | "info" | "warning";

const VARIANT_CLASS: Record<AuthNoticeVariant, string> = {
  error:
...
export function AuthNotice({ variant, children, role }: AuthNoticeProps) {
  return (
    <p
      role={role ?? (variant === "error" ? "alert" : "status")}
      className={`rounded-xl border px-3.5 py-2.5 text-sm leading-relaxed ${VARIANT_CLASS[variant]}`}
    >
      {children}
    </p>
  );
